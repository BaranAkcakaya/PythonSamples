...
#TEKİL MATRİS KONTROLU(DET=0)
def sing_matris(m_1):
  control=False
  if(len(m_1)==4):
    x=det_from_4_by_4(m_1)
    if(x==0):
      control=True
  if(len(m_1)==3):
    x=det_from_3_by_3(m_1)
    if(x==0):
      control=True
  if(len(m_1)==2):
    x=det_from_2_by_2(m_1)
    if(x==0):
      control=True
  return control
print(matris_4,"->",sing_matris(matris_4))
print(matris_2,"->",sing_matris(matris_2))
